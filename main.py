...
import pandas as pd
import helps
from EAs import GA, PSO, CC_GA, CC_PSO
...
if __name__ == "__main__":
    ...
    for run in range(trial_runs):
        ...
        for inst in range(len(tsp_files)):
            ...
            CCPSO_best_path = os.path.dirname(os.path.abspath(__file__)) + "/Data/CCPSO/best_Dis/" + names[inst] + ".csv"
            CCPSO_route_path = os.path.dirname(os.path.abspath(__file__)) + "/Data/CCPSO/Route/" + names[inst] + ".csv"
            CCPSO_trace_path = os.path.dirname(os.path.abspath(__file__)) + "/Data/CCPSO/trace/" + names[inst] + ".csv"
            ...
            time_CCPSO_start = time.time()
            best_Dis_CCPSO, Route_CCPSO, trace_CCPSO = CC_PSO.CC_PSO_exe(cities, NIND, Max_iter, names[inst])
            helps.write_result(CCPSO_best_path, best_Dis_CCPSO)
            helps.write_result(CCPSO_route_path, Route_CCPSO)
            helps.write_result(CCPSO_trace_path, trace_CCPSO)
            time_CCPSO_end = time.time()
            print("CCPSO run time: ", time_CCPSO_end - time_CCPSO_start)
